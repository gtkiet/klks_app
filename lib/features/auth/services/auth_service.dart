...
class AuthService {
  ...
  Future<void> logout() async {
    try {
      // Dùng plainDio để tránh interceptor gọi lại logout khi 401
  ...
  /// Dùng Dio riêng (không qua interceptor) để tránh vòng lặp vô hạn.
  Future<String?> refreshToken({String? refreshToken}) async {
    try {
      final token = refreshToken ?? await _session.getRefreshToken();
      if (token == null || token.isEmpty) return null;

      // Dùng Dio tách biệt — KHÔNG dùng ApiClient.instance.dio
      // vì ApiInterceptor sẽ lại gọi refreshToken() → vòng lặp vô hạn
